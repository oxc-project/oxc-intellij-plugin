...
class OxfmtOnSaveFixAllActionInfo(actionOnSaveContext: ActionOnSaveContext) :
    ActionOnSaveBackedByOwnConfigurable<OxfmtConfigurable>(actionOnSaveContext,
        OxfmtConfigurable.CONFIGURABLE_ID, OxfmtConfigurable::class.java) {

    override fun getActionOnSaveName() =
        OxfmtBundle.message("oxfmt.run.fix.all.on.save.checkbox.on.actions.on.save.page")
    ...
    override fun isActionOnSaveEnabledAccordingToStoredState() =
        OxfmtSettings.getInstance(project).fixAllOnSave
    ...
    override fun setActionOnSaveEnabled(configurable: OxfmtConfigurable, enabled: Boolean) {
        configurable.fixAllOnSaveCheckBox.isSelected = enabled
    }

    override fun getCommentAccordingToUiState(
        configurable: OxfmtConfigurable): ActionOnSaveComment? {
        return comment()
    }
    ...
    override fun getActionLinks() =
        listOf(createGoToPageInSettingsLink(OxfmtConfigurable.CONFIGURABLE_ID))

    private fun comment(): ActionOnSaveComment? {
        if (!isSaveActionApplicable) return ActionOnSaveComment.info(
            OxfmtBundle.message("oxfmt.on.save.comment.disabled"))

        return ActionOnSaveComment.info(OxfmtBundle.message("oxfmt.on.save.comment.fix.all"))
    }
}
