...
Tests can be executed with `./gradlew check`. Prior to running tests, you'll need to execute `pnpm install` within each
of the test directories found at `src/test/testData`.

If you fail to execute `pnpm install` within a test directory, you'll typically receive an exception like the below.
